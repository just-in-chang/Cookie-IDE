...
public class Compiler {
			...
			if (file == null) {
				...
				Thread putOut = new Thread() {
					...
					public void run() {
						try {
							oos.writeByte(0);
							oos.writeObject(workspace.getWidth() + ", " + workspace.getHeight());
							for (Node n : list) {
	...
	private void sendButton(ObjectOutputStream oos, Bounds boundsInScene, Node n) throws Exception {
	...
	private void sendLabel(ObjectOutputStream oos, Bounds boundsInScene, Node n) throws Exception {
		oos.writeObject("Labeled");
		oos.writeObject(((Labeled) n).getText());
		oos.writeObject(Double.toString(boundsInScene.getMinX()));
		oos.writeObject(Double.toString(boundsInScene.getMinY()));
	...
	private void sendTextField(ObjectOutputStream oos, Bounds boundsInScene, Node n) throws Exception {
